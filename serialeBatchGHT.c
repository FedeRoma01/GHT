...
unsigned char* rotate_image_nearest_neighbor_expand(unsigned char *src, int width, int height, float angle_degrees, int *new_width, int *new_height) {
    float angle_radians = -angle_degrees * (M_PI / 180.0f);   // - for clockwise
...
int main(int argc, char **argv) {
    ...
    for (int a = 0; a < NUM_ANGLES; a++) {
        ...
        int tw, th;
        unsigned char *rotated = rotate_image_nearest_neighbor_expand(template_img, twi, thi, angle, &tw, &th);
