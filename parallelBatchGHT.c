...
void generalized_hough(unsigned char *edges, float *grad_x, float *grad_y, int width, int height, int start_row, int end_row, int rank, Point **finalDetections, int *detectionsCounter, FILE *profile_fp, int ai) {
    ...
    Point *detections = NULL;
    if (rank == 0) detections = calloc(width * height, sizeof(Point)); // max one point per cell
    ...
    if (rank == 0)
        global_accumulator = calloc(acc_w * acc_h, sizeof(int));
...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < num_files; i++) {
        ...
        for (int ai = 0; ai < NUM_ANGLES; ai++) {
            ...
            timestamp_start = MPI_Wtime();
            generalized_hough(edges, grad_x, grad_y, scene_w, scene_h, start_row, end_row, rank, &finalDetections, &detectionsCounter, profile_fp, ai);
            timestamp_end = MPI_Wtime();
