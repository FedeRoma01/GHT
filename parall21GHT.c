...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        scene_img = load_image_dynamic("resources/scene_key.pgm", &scene_w, &scene_h);
    ...
    float *grad_x = NULL, *grad_y = NULL, *magnitude = NULL;
    if (rank == 0) {
        grad_x = (float*)malloc(scene_w * scene_h * sizeof(float));
        grad_y = (float*)malloc(scene_w * scene_h * sizeof(float));
        magnitude = (float*)malloc(scene_w * scene_h * sizeof(float));
    }

    timestamp_start = MPI_Wtime();
    compute_gradient_mpi(scene_img, grad_x, grad_y, magnitude, scene_w, scene_h, MPI_COMM_WORLD);
    timestamp_end = MPI_Wtime();
    if (rank==0) fprintf(profile_fp,"compute_gradient_mpi: %.6f s\n", timestamp_end - timestamp_start);

    unsigned char *edges = NULL;
    if (rank == 0) {
        edges = (unsigned char*)malloc(scene_w*scene_h);
        timestamp_start = MPI_Wtime();
    ...
    // Template
    unsigned char *template_img=NULL; int tw=0, th=0;
    if (rank==0) {
        timestamp_start = MPI_Wtime();
    ...
    for (int ai=0; ai<NUM_ANGLES; ai++) {
        if (rank == 0) {
            ...
            timestamp_start = MPI_Wtime();
            rotated = rotate_image_nearest_neighbor_expand(template_img, tw, th, angles[ai], &rtw, &rth);
            timestamp_end = MPI_Wtime();
            fprintf(profile_fp,"rotate_image: %.6f s\n", timestamp_end - timestamp_start);

            float *tgrad_x=(float*)malloc(rtw * rth * sizeof(float));
            float *tgrad_y=(float*)malloc(rtw * rth * sizeof(float));
            float *tmag=(float*)malloc(rtw * rth * sizeof(float));
            unsigned char *tedges=(unsigned char*)malloc(rtw * rth);

            timestamp_start = MPI_Wtime();
            compute_gradient_local(rotated, tgrad_x, tgrad_y, tmag, rtw, rth);
            timestamp_end = MPI_Wtime(); 
            fprintf(profile_fp, "compute_gradient: %.6f s\n", timestamp_end - timestamp_start);

            timestamp_start = MPI_Wtime();
            detect_edges(tmag, tedges, rtw, rth);
            timestamp_end = MPI_Wtime(); 
            fprintf(profile_fp, "detect_edges: %.6f s\n", timestamp_end - timestamp_start);

            timestamp_start = MPI_Wtime();
            build_lookup_table(tedges, tgrad_x, tgrad_y, rtw, rth);
            timestamp_end = MPI_Wtime(); 
            fprintf(profile_fp, "build_lookup_table: %.6f s\n", timestamp_end - timestamp_start);

            free(tgrad_x); free(tgrad_y); free(tmag); free(tedges); free(rotated);

            Point *finalDetections=NULL; int detectionsCounter=0;
            timestamp_start= MPI_Wtime();
            generalized_hough(edges, grad_x, grad_y, scene_w, scene_h, &finalDetections, &detectionsCounter);
            timestamp_end= MPI_Wtime();
            fprintf(profile_fp,"generalized_hough (%.0f): %.6f s\n", angles[ai], timestamp_end - timestamp_start);
    ...
    if (rank==0) {
        free(scene_img); free(template_img);
        free(grad_x); free(grad_y); free(magnitude); free(edges);
        fclose(profile_fp);
    }
