...
void generalized_hough(unsigned char *edges, float *grad_x, float *grad_y, int width, int height, int start_row, int end_row, int rank, Point **finalDetections, int *detectionsCounter, FILE *profile_fp) {
    ...
    int *global_accumulator = calloc(acc_w * acc_h, sizeof(int));
    ...
    if (rank == 0) {
        ...
        for (int y = 0; y < acc_h; y++) {
            for (int x = 0; x < acc_w; x++) {
                if (global_accumulator[y * acc_w + x] > VOTE_THRESHOLD) {
        ...
        for (int i = 0; i < num_det; i++) {
            ...
            for (int j = 0; j < num_det; j++) {
                ...
                if (distance < MIN_DISTANCE) {
                    int acc_i = global_accumulator[(detections[i].y / DP) * acc_w + (detections[i].x / DP)];
                    int acc_j = global_accumulator[(detections[j].y / DP) * acc_w + (detections[j].x / DP)];
                    if (acc_i < acc_j) {
                        isMax = 0;
                        break;
                    }
...
int main(int argc, char **argv) {
    ...
    for (int ai = 0; ai < NUM_ANGLES; ai++) {
        ...
        if (rank == 0) {   
            ...
            int y = th;
            timestamp_start = MPI_Wtime();
            rotated = rotate_image_nearest_neighbor_expand(template_img, x, y, angles[ai], &tw, &th);
            timestamp_end = MPI_Wtime();
        ...
        // Broadcast of the look-up table
        timestamp_start = MPI_Wtime();
        ...
        // Voting phase
        timestamp_start = MPI_Wtime();
        generalized_hough(edges, grad_x, grad_y, scene_w, scene_h, start_row, end_row, rank, &finalDetections, &detectionsCounter, profile_fp);
        timestamp_end = MPI_Wtime();
        fprintf(profile_fp, "generalized_hough (a=%.0f): %.6f s\n", angles[ai], timestamp_end - timestamp_start);

        // If something valid is detected -> save overlay detection
        if (rank == 0 && detectionsCounter > 0) {
