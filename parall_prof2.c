...
void generalized_hough(unsigned char *edges, float *grad_x, float *grad_y, int *accumulator, int width, int height) {
    for (int y = 1; y < height - 1; y++) {
        for (int x = 1; x < width - 1; x++) {
            int idx = y * width + x;
            if (edges[idx] == 255) {
...
int main(int argc, char **argv) {
    ...
    float *grad_x = calloc(scene_size, sizeof(float));
    float *grad_y = calloc(scene_size, sizeof(float));
    float *magnitude = calloc(scene_size, sizeof(float));
    unsigned char *edges = calloc(scene_size, sizeof(unsigned char));
    ...
    for (int t = 1; t <= num_templates; t++) {
        ...
        // WORK LOAD PER PROCESS (W.R.T 1 TEMPLATE)
        int total_combinations = NUM_SCALES * NUM_ANGLES;
        int start = rank * total_combinations / size;
        int end = (rank + 1) * total_combinations / size;

        // PRELIMINARY TEMPLATE EDGES COMPUTATION
        float *tgrad_x;
        float *tgrad_y;
        float *tmagnitude;
        ...
        free(tgrad_y);
        free(tmagnitude);
        // t_edges used as base for any transformation

        for (int idx = start; idx < end; idx++) {
            int si = idx / NUM_ANGLES;
            int ai = idx % NUM_ANGLES;
            float scale = scales[si];
            float angle = angles[ai];

            int tw_s = (int)(tw * scale);
            int th_s = (int)(th * scale);

            unsigned char *resized = calloc(tw_s * th_s, sizeof(unsigned char));
            unsigned char *rotated = calloc(tw_s * th_s, sizeof(unsigned char));
            stbir_resize_uint8(t_edges, tw, th, 0, resized, tw_s, th_s, 0, 1);
            int x = tw_s;
            int y = th_s;
            rotated = rotate_image_bilinear_expand(resized, x, y, angle, &tw_s, &th_s);
            free(resized);
        
            float *tgrad_x = calloc(tw_s * th_s, sizeof(float));
            float *tgrad_y = calloc(tw_s * th_s, sizeof(float));
            float *tmagnitude = calloc(tw_s * th_s, sizeof(float));
            unsigned char *tedges = calloc(tw_s * th_s, sizeof(unsigned char));
            ...
            timestamp_start = MPI_Wtime();
            build_lookup_table(tedges, tgrad_x, tgrad_y, tw_s, th_s);
            timestamp_end = MPI_Wtime();
            ...
            timestamp_start = MPI_Wtime();
            generalized_hough(edges, grad_x, grad_y, local_accumulator, scene_w, scene_h);
            timestamp_end = MPI_Wtime();
            ...
            timestamp_start = MPI_Wtime();
            non_maximum_suppression(local_accumulator, nms_result, scene_w, scene_h, window_size);
            timestamp_end = MPI_Wtime();
    ...
    free(edges); free(local_accumulator); free(nms_result);
