...
int main(int argc, char **argv) {
    ...
    int tw = 0, th = 0;
    if (rank == 0) lookup_tables = malloc(NUM_ANGLES * sizeof(LookupTable));
    ...
    if (rank == 0) {
        ...
        // PRE-PROCESSING LOOKUP TABLES (only on rank 0)
        for (int a = 0; a < NUM_ANGLES; a++) {
            ...
            timestamp_start = MPI_Wtime();
            rotated = rotate_image_nearest_neighbor_expand(template_img, tw, th, angles[a], &rtw, &rth);
            timestamp_end = MPI_Wtime();
            fprintf(profile_fp, "rotate_image_nearest_neighbor_expand (a=%.0f): %.6f s\n", angle, timestamp_end - timestamp_start);
            ...
            timestamp_start = MPI_Wtime();
            build_lookup_table(tedges, tgrad_x, tgrad_y, rtw, rth, &lookup_tables[a]);
            timestamp_end = MPI_Wtime();
    ...
    for (int i = 0; i < num_files; i++) {
        ...
        if (rank == 0) {
            edges = (unsigned char*)malloc(scene_size);
            timestamp_start = MPI_Wtime();
